...
def fetch_hcp_history_tool(hcp_name: str) -> str:
    ...
    try:
        ...
        interactions = db.query(Interaction).filter(Interaction.hcp_id == target_hcp.id).order_by(Interaction.interaction_date.desc()).all()
        
        open_follow_ups = db.query(FollowUp).join(Interaction).filter(
        ...
        for fu in open_follow_ups:
            follow_ups_data.append({
                "due_date": str(fu.due_date) if fu.due_date else None,
                "note": fu.note
            })
        ...
        result = {
            ...
            "last_visit_date": str(target_hcp.last_interaction_date) if target_hcp.last_interaction_date else "Never",
            "recent_interactions": history[:5], # latest 5
